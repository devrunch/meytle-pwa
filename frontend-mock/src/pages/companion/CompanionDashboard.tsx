...
function buildChartData(bookings: ApiBooking[]): { day: string; val: number }[] {
  const labels = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
  const counts = [0, 0, 0, 0, 0, 0, 0]
  bookings.forEach(b => {
    const d = new Date(b.bookedStart)
    const i = (d.getDay() + 6) % 7 // convert Sun=0 to Mon=0 indexing
    counts[i]++
  })
  const max = Math.max(...counts, 1)
  return labels.map((day, i) => ({ day, val: Math.round((counts[i] / max) * 100) }))
}
...
export default function CompanionDashboard() {
  ...
  const [companionRating, setCompanionRating] = useState<string | null>(null)
  const [companionRatingCount, setCompanionRatingCount] = useState(0)
  const [companionName, setCompanionName] = useState('')
  const [companionAvatar, setCompanionAvatar] = useState<string | null>(null)
  const [companionBio, setCompanionBio] = useState<string | null>(null)
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get<{ displayName: string; profilePhotoUrl: string | null; bio: string | null; profileStatus: string; ratingAvg: number | null; ratingCount: number }>('/companions/me/profile'),
    ]).then(([bookingsRes, profileRes]) => {
      const bookings = bookingsRes.data
      setRequests(bookings.map(toRequest))
      setChartData(buildChartData(bookings))
      const pending = bookings
        .filter(b => b.status === 'confirmed' || b.status === 'in_progress')
        .reduce((sum, b) => sum + Math.round(b.amountPaisa / 100), 0)
      setPendingPayout(pending)
      setCompanionName(profileRes.data.displayName)
      setCompanionAvatar(profileRes.data.profilePhotoUrl)
      setCompanionBio(profileRes.data.bio)
      setCompanionStatus(profileRes.data.profileStatus)
      setCompanionRating(profileRes.data.ratingAvg != null ? Number(profileRes.data.ratingAvg).toFixed(1) : null)
      setCompanionRatingCount(profileRes.data.ratingCount ?? 0)
  ...
  const statsWithCounts = [
    ...
    {
      label: 'Bookings',
      value: requests.length > 0 ? String(requests.length) : '—',
    ...
    {
      label: 'Rating',
      value: companionRating ?? '—',
      sub: `${companionRatingCount} review${companionRatingCount !== 1 ? 's' : ''}`,
      positive: true,
  ...
  return (
    <div className="min-h-full bg-[var(--color-bg)]">
                  ...
                  {companionStatus && (
                    <div className={`flex items-center gap-1 rounded-full px-2 py-0.5 ${
                      companionStatus === 'active'
                        ? 'bg-white/20'
                        : companionStatus === 'pending_verification'
                        ? 'bg-yellow-400/30'
                        : 'bg-red-400/30'
                    }`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${
                        companionStatus === 'active' ? 'bg-[#4ade80]'
                        : companionStatus === 'pending_verification' ? 'bg-yellow-300'
                        : 'bg-red-300'
                      }`} />
                      <span className="text-[10px] text-white font-semibold">
                        {companionStatus === 'active' ? 'Active'
                          : companionStatus === 'pending_verification' ? 'Pending review'
                          : companionStatus === 'inactive' ? 'Inactive'
                          : 'Rejected'}
                      </span>
                    </div>
                  )}
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-6 lg:px-10 py-4">
        ...
        {/* Status banner — shown when not active */}
        {companionStatus && companionStatus !== 'active' && (
          <div className={`mb-4 rounded-[12px] border px-4 py-3 flex items-start gap-3 ${
            companionStatus === 'pending_verification'
              ? 'bg-yellow-50 border-yellow-200'
              : companionStatus === 'rejected'
              ? 'bg-red-50 border-red-200'
              : 'bg-[var(--color-gray-light)] border-[var(--color-border)]'
          }`}>
            <IconAlertCircle size={16} stroke={1.5} className={`mt-0.5 flex-shrink-0 ${
              companionStatus === 'pending_verification' ? 'text-yellow-600'
              : companionStatus === 'rejected' ? 'text-red-500'
              : 'text-[var(--color-gray)]'
            }`} />
            <div>
              <p className={`text-[13px] font-semibold ${
                companionStatus === 'pending_verification' ? 'text-yellow-800'
                : companionStatus === 'rejected' ? 'text-red-700'
                : 'text-[var(--color-dark)]'
              }`}>
                {companionStatus === 'pending_verification' && 'Profile under review — not yet searchable'}
                {companionStatus === 'rejected' && 'Profile not approved'}
                {companionStatus === 'inactive' && 'Profile is currently hidden'}
              </p>
              <p className={`text-[12px] mt-0.5 ${
                companionStatus === 'pending_verification' ? 'text-yellow-700'
                : companionStatus === 'rejected' ? 'text-red-600'
                : 'text-[var(--color-gray)]'
              }`}>
                {companionStatus === 'pending_verification' && 'Your profile is being reviewed by our team. We\'ll notify you once it\'s approved and live.'}
                {companionStatus === 'rejected' && 'Your profile was not approved. Please contact support for details.'}
                {companionStatus === 'inactive' && 'Your profile is hidden from search. Contact support to reactivate.'}
              </p>
            </div>
          </div>
        )}
            ...
            <div className="bg-white rounded-[14px] border border-[var(--color-border)] p-4">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-[var(--color-border)]">
                <div>
                  ...
                  <p className="text-[16px] font-bold text-[var(--color-dark)]">{totalEarned > 0 ? `₹${totalEarned.toLocaleString()}` : '—'}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-[var(--color-gray)]">Pending payout</p>
                  <p className="text-[16px] font-bold text-[var(--color-dark)]">{pendingPayout > 0 ? `₹${pendingPayout.toLocaleString()}` : '—'}</p>
                </div>
              </div>
            </div>

            {(() => {
              const hasPhoto = !!companionAvatar
              const hasBio = !!companionBio
              const isActive = companionStatus === 'active'
              const profileItems = [
                { label: 'Profile photo',   done: hasPhoto,  icon: <IconUsers size={12} stroke={1.5} /> },
                { label: 'Bio written',     done: hasBio,    icon: <IconShieldCheck size={12} stroke={1.5} /> },
                { label: 'Profile verified', done: isActive, icon: <IconCalendarEvent size={12} stroke={1.5} /> },
              ]
              const strength = Math.round((profileItems.filter(i => i.done).length / profileItems.length) * 100)
              return (
                <div className="bg-white rounded-[14px] border border-[var(--color-border)] p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-[13px] font-semibold text-[var(--color-dark)]">Profile Strength</h3>
                    <span className="text-[11px] text-[var(--color-amber)] font-medium">{strength}%</span>
                  </div>
                  <div className="h-1.5 bg-[var(--color-gray-light)] rounded-full mb-3 overflow-hidden">
                    <div className="h-full rounded-full transition-all" style={{ width: `${strength}%`, background: 'var(--gradient-gold)' }} />
                  </div>
            ...
            <div className="bg-white rounded-[14px] border border-[var(--color-border)] overflow-hidden">
              ...
              <div className="p-4">
                {loading ? (
                  ...
                  tab === 'requests' ? (
                    <EmptyState
                      icon={<IconInbox size={28} stroke={1.2} />}
                      title="All clear!"
                      body="New booking requests will appear here"
                    />
                  ) : (
                    <EmptyState
                      icon={<IconCalendarEvent size={28} stroke={1.2} />}
                      title="No upcoming bookings"
                      body="Accepted bookings will appear here"
                    />
                  )
                ) : (
                  <>
